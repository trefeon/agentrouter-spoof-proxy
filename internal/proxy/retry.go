...
// deterministic content-filter rejection rather than an outage: a 5xx status
// whose body carries the `sensitive_words` marker, or a 400 status whose body
// carries the `content-blocked` marker. Such rejections are client-side (the
// prompt hit the filter), never evidence of an upstream outage, so callers
// must skip retry, health marks, and breaker failure accounting. Matching is
// case-sensitive.
func IsContentFilterRejection(statusCode int, body []byte) bool {
	if statusCode == http.StatusBadRequest {
		return strings.Contains(string(body), "content-blocked")
	}
	if statusCode < 500 || statusCode > 599 {
		return false
	}
